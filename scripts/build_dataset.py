...
CANADA_PARKS_PATH = Path(
    "data/reference/canada_national_parks.geojson"
)

US_PARKS_PATH = Path(
    "data/reference/us/"
    "Administrative_Boundaries_of_"
    "National_Park_System_Units.shp"
)
...
TEMP_OUTPUT_PATH = Path(
    "public/data/observations.tmp.parquet"
)
            ...
            for month in range(1, 13):
                ...
                while downloaded < RECORDS_PER_MONTH:
                    ...
                    if not isinstance(payload, dict):
                        raise RuntimeError(
                            "GBIF returned an invalid response."
                        )
...
def clean_records(
    ...
    for record in records:
        ...
        country_code = str(
            ...
            or record.get("_requested_country")
            or ""
        ).strip().upper()
...
def load_protected_areas() -> gpd.GeoDataFrame:
    ...
    for path, name_column in sources:
        if not path.exists():
            raise RuntimeError(
                f"Protected-area file was not found: {path}"
            )
        ...
        if boundaries.crs is None:
            raise RuntimeError(
                f"Protected-area file has no CRS: {path}"
            )

        if name_column not in boundaries.columns:
            raise RuntimeError(
                f"Expected column {name_column} "
                f"was not found in {path}."
            )

        boundaries = boundaries[
            [name_column, "geometry"]
        ].rename(
            columns={
                name_column: "protected_area_name"
            }
        )
    ...
    if protected_areas.empty:
        raise RuntimeError(
            "No protected-area boundaries were loaded."
        )
...
def add_protected_area_context(
    ...
    if observations.crs is None:
        raise RuntimeError(
            "Observation data has no CRS."
        )

    if protected_areas.crs is None:
        raise RuntimeError(
            "Protected-area data has no CRS."
        )

    observations = observations.to_crs(
        "EPSG:4326"
    )

    protected_areas = protected_areas.to_crs(
        "EPSG:4326"
    )
...
def validate_dataset(
    ...
    if dataset.empty:
        raise RuntimeError(
            "No valid observations were produced."
        )

    if len(dataset) > MAX_TOTAL_RECORDS:
        raise RuntimeError(
            "The cleaned dataset exceeded "
    ...
    if dataset["observation_id"].duplicated().any():
        raise RuntimeError(
            "The dataset contains duplicate IDs."
        )

    if not dataset["country_code"].isin(
        COUNTRIES
    ).all():
        raise RuntimeError(
            "The dataset contains an "
            "unexpected country code."
        )

    if not dataset["latitude"].between(
        -90,
        90,
    ).all():
        raise RuntimeError(
            "The dataset contains invalid latitude."
        )

    if not dataset["longitude"].between(
        -180,
        180,
    ).all():
        raise RuntimeError(
            "The dataset contains invalid longitude."
        )

    if dataset.crs is None:
        raise RuntimeError(
            "The dataset has no coordinate system."
        )

    if dataset.crs.to_epsg() != 4326:
        raise RuntimeError(
            "The dataset has the wrong "
            "coordinate system."
        )
    ...
    if not inside.isin(
        ...
        raise RuntimeError(
            "Protected-area flags are invalid."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "A protected-area observation "
            "has no protected-area name."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "An observation outside protected "
            "areas has a protected-area name."
        )
...
def write_dataset(
    ...
    try:
        ...
        saved_dataset = gpd.read_parquet(
            TEMP_OUTPUT_PATH
        )
