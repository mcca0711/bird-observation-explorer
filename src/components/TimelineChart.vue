...
const monthlyCounts = computed(() => {
  const counts = monthNames.map((label, index) => ({
    month: index + 1,
    label,
    count: 0,
  }))
...
const totalRecords = computed(() =>
  monthlyCounts.value.reduce((total, item) => total + item.count, 0),
)

const representedMonthCount = computed(
  () => monthlyCounts.value.filter((item) => item.count > 0).length,
)

const highestCount = computed(
  () => max(monthlyCounts.value, (item) => item.count) ?? 0,
)
...
const peakMonths = computed(() => {
  ...
  return monthlyCounts.value.filter((item) => item.count === highestCount.value)
})

function formatMonthList(labels: string[]): string {
  if (labels.length === 0) {
    return ''
  }
...
const peakSummary = computed(() => {
  const labels = peakMonths.value.map((item) => item.label)

  const countLabel =
    highestCount.value === 1 ? '1 record' : `${highestCount.value} records`

  if (labels.length === 1) {
    return `Peak month ${labels[0]} ` + `with ${countLabel}.`
  }

  return `Peak months ${formatMonthList(labels)} ` + `with ${countLabel} each.`
})

const xScale = computed(() =>
  scaleBand<number>()
    .domain(monthlyCounts.value.map((item) => item.month))
...
const yScale = computed(() =>
  scaleLinear()
    .domain([0, Math.max(1, highestCount.value)])
    .nice()
...
const yTicks = computed(() => {
  ...
  if (highest <= 5) {
    return Array.from(
      {
        length: Math.max(1, highest) + 1,
      },
      (_, index) => index,
    )
  }

  const step = Math.ceil(highest / 4)

  const ticks: number[] = []

  for (let value = 0; value <= highest; value += step) {
    ticks.push(value)
  }

  if (ticks[ticks.length - 1] !== highest) {
    ticks.push(highest)
  }
...
function barX(month: number): number {
  return xScale.value(month) ?? chartLeft
}
...
function barHeight(count: number): number {
  return chartBottom - yScale.value(count)
}

function selectMonth(month: number, count: number): void {
  if (count === 0) {
    return
  }

  emit('selectMonth', props.selectedMonth === month ? null : month)
}

function handleMonthChange(event: Event): void {
  const value = (event.target as HTMLSelectElement).value

  emit('selectMonth', value === 'all' ? null : Number(value))
}
</script>

<template>
  <section class="border-t border-white/10 bg-slate-950 p-4 sm:p-5">
    <div
    ...
    <template v-else>
      ...
      >
        <div>
          ...
          <p v-if="highestCount > 0" class="mt-1 text-sm text-slate-400">
            {{ peakSummary }}
          </p>

          <p
            v-if="totalRecords > 0 && totalRecords < 20"
        ...
        <div class="flex flex-wrap items-end gap-3">
          <label class="block">
            <span class="mb-1 block text-xs font-medium text-slate-400">
              Filter map by month
            </span>
      ...
      <svg
        ...
        class="h-auto w-full"
        role="img"
        :aria-label="`Monthly sampled records for ${speciesName}`"
      >
        <desc>
        ...
        <g
          ...
          :aria-label="`${item.label}, ${item.count} sampled records`"
          :class="item.count > 0 ? 'cursor-pointer' : ''"
          @click="selectMonth(item.month, item.count)"
          @keydown.enter="selectMonth(item.month, item.count)"
          @keydown.space.prevent="selectMonth(item.month, item.count)"
        >
          <rect
            :x="barX(item.month)"
            :y="barY(item.count)"
          ...
          <text
            v-if="item.count > 0"
            :x="barX(item.month) + xScale.bandwidth() / 2"
            :y="barY(item.count) - 9"
            text-anchor="middle"
