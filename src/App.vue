...
const selectedSpeciesName = computed(() => {
  ...
  const observation = observations.value.find(
    item =>
      item.scientificName === selectedSpecies.value,
  )
...
const protectedAreaObservations = computed(() => {
  if (selectedProtectedArea.value === 'inside') {
    return speciesObservations.value.filter(
      observation =>
        observation.insideProtectedArea,
    )
  }
...
<template>
  ...
  >
    <header
      class="border-b border-white/10 px-4 py-6 sm:px-6"
    >
      <p
    ...
    >
      ...
      >
        ...
        <p
          class="mt-6 text-xs leading-5 text-slate-500"
        >
      ...
      <section class="min-w-0 bg-slate-900">
        <div
          v-if="loading"
          class="grid min-h-[500px] place-items-center"
        >
